...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Spotinst").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-jet-spotinst"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-jet-spotinst",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Spotinst APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Spotinst controllers")
